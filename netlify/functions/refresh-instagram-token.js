...
exports.handler = async () => {
  ...
  try {
    const refreshRes = await fetch(
      `https://graph.instagram.com/refresh_access_token?grant_type=ig_refresh_token&access_token=${longToken}`
    );
    ...
    const updateRes = await fetch(
      `https://api.netlify.com/api/v1/sites/${siteId}/env/INSTAGRAM_GRAPH_ACCESS_TOKEN`,
      {
        method: 'PATCH',
        headers: {
